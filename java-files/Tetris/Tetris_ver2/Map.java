...
public class Map extends JPanel implements KeyListener, ActionListener {
    ...
    private void setTile(int x, int y, int label) {
        int p_x = x * TILE_W;
        int p_y = y * TILE_H;
        fill(p_x, p_y, TILE_W, TILE_H, label);
        // System.out.print(p_x + " : " + p_y + "\n");
    }

    private void fill(int x_l, int y_u, int t_w, int t_h, int label) {
    ...
    private void viewMap() {
        ...
        for (int j = 0; j < MINIMAP_H * TILE_H; j++) {
            for (int i = 0; i < MINIMAP_W * TILE_W; i++) {
                map[j + OFFSET_Y][i + OFFSET_X] = minimap[j / TILE_H][i / TILE_W];
            }
        }

        for (int j = 0; j < MINO_H; j++) {
            for (int i = 0; i < MINO_W; i++) {
                if (mino.arr[mino.mode][j][i] == 1 && minimap[mino.pos[1] + j][mino.pos[0] + i] == 0)
