...
double followOutlineAndGetLength(int** ori, int** buf, int width, int height, int* dc, int x, int y, int label) {
    ...
    do {
        ...
        for (int i = 0; i < DIRECTION_MAX_INTEGER; i++) {
            if (i)
                dn = (dn + 1 + DIRECTION_MAX_INTEGER) % DIRECTION_MAX_INTEGER;
            switch (dn) {
            case 0:
                if (x_c < width - 1 && ori[y_c + 0][x_c + 1]) {
                x_c++;
                skip_frag++;
                }
                break;
            case 1:
                if (x_c < width - 1 && y_c > 0 && ori[y_c - 1][x_c + 1]) {
                x_c++;
                y_c--;
                skip_frag++;
                }
                break;
            case 2:
                if (y_c > 0 && ori[y_c - 1][x_c + 0]) {
                y_c--;
                skip_frag++;
                }
                break;
            case 3:
                if (x_c > 0 && y_c > 0 && ori[y_c - 1][x_c - 1]) {
                x_c--;
                y_c--;
                skip_frag++;
                }
                break;
            case 4:
                if (x_c > 0 && ori[y_c + 0][x_c - 1]) {
                x_c--;
                skip_frag++;
                }
                break;
            case 5:
                if (x_c > 0 && y_c < height - 1 && ori[y_c + 1][x_c - 1]) {
                x_c--;
                y_c++;
                skip_frag++;
                }
                break;
            case 6:
                if (y_c < height - 1 && ori[y_c + 1][x_c + 0]) {
                y_c++;
                skip_frag++;
                }
                break;
            case 7:
                if (x_c < width - 1 && y_c < height - 1 && ori[y_c + 1][x_c + 1]) {
                x_c++;
                y_c++;
                skip_frag++;
                }
...
int main(void) {
  ...
  sprintf(fnameout, "%s-fvalue-out.pgm", tok);
