...
public class Server extends Thread {
    ...
    public synchronized void sendAll(int[][] message, int aabanme, int mposX, int mposY, int mmode, int mlabel,
            int next1, int next2, int next3, int next4, int mreserve) {
    ...
    // damage
    public synchronized void sendAll(int banme, int message) {
        for (int i = 0; i < socketThreads.size(); i++)
            socketThreads.get(i).sendMessage(banme, message);
    }
    ...
    static class ServerSocketThread extends Thread {
                ...
                while (isWorkingServer) {
                    ...
                    if (word.length == 2) {
                        ...
                        if (word[0].charAt(0) == '2') {
                            server.sendAll(word[0].charAt(1) - '0', word[1].charAt(0) - '0');
                            System.out.println(word[1].charAt(0) - '0');
                        }
                        if (word[0].charAt(0) == '3') {
                            ...
                            if ("BACKTO-C-ST".equals(word[1])) server.sendAll(word[1]);
        ...
        public synchronized void sendMessage(int[][] screen, int abanme, int mposX, int mposY, int mmode, int mlabel,
            ...
            try {
                output.println("1" + (char) (abanme + '0') + (char) (mposX + '0') + (char) (mposY + '0')
                        ...
                        + (char) (next3 + '0') + (char) (next4 + '0') + (char) (mreserve + '0') + "#"
        ...
        // damage
        public synchronized void sendMessage(int banme, int num) {
            try {
