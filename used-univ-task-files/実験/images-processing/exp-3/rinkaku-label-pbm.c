...
int main(void)
{
  ...
  sprintf(fnameout, "%s-label-out.pgm", tok);
  ...
  for (y = 1; y < height; y++) {
    for (x = 1; x < width; x++) {
        if(!bufArr[y][x] && bufArr[y][x-1] && img[y][x]) bufArr[y][x] = bufArr[y][x-1];
        if(!bufArr[y][x] && bufArr[y-1][x] && img[y][x]) bufArr[y][x] = bufArr[y-1][x];
    }
  }
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      img[y][x] = bufArr[y][x] * (maxvalue / label);
    }
  }
