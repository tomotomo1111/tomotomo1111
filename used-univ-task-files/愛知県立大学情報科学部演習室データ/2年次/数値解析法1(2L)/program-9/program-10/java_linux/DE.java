...
public class DE {
	...
	double DEBD(double a, double b, double h, int M, int N, double[] f)
	{
		...
		else
		{
			...
			for (l = -M; l <= N; l++) {
				t = l * h;
				tanhval = Math.tanh(Math.PI / 2 * Math.sinh(t));
				rval += f[l + M] * (1 - Math.pow(tanhval, 2)) * ((Math.PI * r * Math.cosh(t)) / 2);
			}
			rval *= h;
		}
	...
	double DEIF(double h, int M, int N, double[] f)
	{
		...
		else
		{
			...
			for (l = -M; l <= N; l++) {
				t = l*h;
				rval += f[l + M] * (Math.PI / 2 * Math.cosh(t) * Math.cosh(Math.PI * Math.sinh(t) / 2));
			}
			rval *= h;
		}
