...
int main(void) {
  ...
  scanf("%s", fnamein);
  for (i = 0; i < sizeof(fnamein_buf); i++) fnamein_buf[i] = fnamein[i];

  tok = strtok(fnamein_buf, ".");
  sprintf(fnameout, "%s-heikatu-out.", tok);
  tok = strtok(NULL, ".");
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      ...
      for (j = y_u; j <= y_d; j++) {
        for (i = x_l; i <= x_r; i++) {
          value_sum += img[y + j][x + i];
          count++;
        }
      }
      bufArr[y][x] = value_sum / count;
    }
  }
