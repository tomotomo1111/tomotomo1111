...
\lstinputlisting[caption=velo\_id\_gain.m, label=id]{./velo\_id\_gain.m}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[width=100mm]{wk3/p1p2-equal-crop.pdf}
	\end{center}
	\caption{実数部のみ調整}
	\label{fig:kadai7}
\end{figure}
...
\lstinputlisting[caption=velo\_pi\_mbd.m, label=pi]{./velo\_pi\_mbd.m}
...
\subsection{考察}
%
$p_{1}= -2-a*i, p_{2}=-2+a*i$の$a$に$0.5、 1、2、3$を代入して、虚軸に平行に変化させ、応答の変化を観察する。
