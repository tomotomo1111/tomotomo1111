...
int main(void) {
    ...
    sprintf(fnameout, "%s-senei-out.", tok);
    tok = strtok(NULL, ".");
    ...
    int x_l, y_u;
    int x_r, y_d;
    int value_sum, sharpen_weight;
    int weight;

    do {
        puts("write sharpen-weight under 0. Example -1");
        scanf("%d", &sharpen_weight);
    } while (sharpen_weight >= 0);
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            for (j = y_u; j <= y_d; j++) {
                for (i = x_l; i <= x_r; i++) {
                    weight = 0;
                    if (!j || !i) weight = sharpen_weight;
                    if (!j && !i) weight = 1 - count * sharpen_weight;
                    value_sum += (weight * img[y + j][x + i]);
                }
            }
