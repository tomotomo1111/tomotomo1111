...
public class CSpline {
	...
	double Interpolation(double xcalc)
	{
		...
		else
		{
			...
			rval = f[i-1] * ((((x[i] - xcalc) * (x[i] - xcalc)) * (2 * (xcalc - x[i-1]) + h)) / Math.pow(h, 3));
			rval += f[i] * ((((xcalc - x[i-1]) * (xcalc - x[i-1])) * (2 * (x[i] - xcalc) + h)) / Math.pow(h, 3));
			rval += c[i-1] * ((((x[i] - xcalc) * (x[i] - xcalc)) * (xcalc - x[i-1])) / (Math.pow(h, 2)));
			rval -= c[i] * (((x[i] - xcalc) * ((xcalc - x[i-1]) * (xcalc - x[i-1]))) / (Math.pow(h, 2)));
	...
	void SplineCoef(double[] xp, double[] fp, double[] dfp)
	{
		...
		else
		{
			...
			for (i = 1; i < n; i++) {
				...
				beta[i] = 2 * ((1 / h1) + (1 / h2));
				gamma[i] = 1 / h2;
				c[i] = 3 * ((f[i] - f[i - 1]) / Math.pow(h1, 2) + (f[i + 1] - f[i]) / Math.pow(h2, 2));
			}
			beta[0] = beta[n] = 1;
			alpha[n - 1] = 0;
			gamma[0] = 0;
